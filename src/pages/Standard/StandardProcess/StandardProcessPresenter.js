...
const StandardProcessPresenter = ({
  ...
  const codeFilter = [...new Set(standardprocesses.map(StandardProcess => StandardProcess.spCode))].map(code =>({
    text : code,
    value : code
  }));

  const nameFilter = [...new Set(standardprocesses.map(StandardProcess => StandardProcess.spName))].map(name =>({
    text : name,
    value : name
  }));

  const timeFilter = [... new Set(standardprocesses.map(StandardProcess => StandardProcess.spTime))].map(time => ({
    text : time,
    value : time
  }));
  ...
  return(
    <>
        <div className="standardprocess-content">
            <div className="grid-func">
                ...
                <div className="func-button">
                   ...
                   <Button type="primary" danger  disabled={!hasSelected} onClick={HandleDeleteStandardProcess}>삭제</Button>                
                </div>
            </div>
            ...
            <div className="grid-box">
                  <Table size="small"
                   ...
                   columns={columns} 
                   dataSource={isSearching ? filteredStandardProcesses : standardprocesses} 
                   rowKey="spNo" 
                   onRow={(record) => ({
     ...
     <Form labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} onFinish={HandleCreateStandardProcess}>
      <Form.Item label="표준공정코드">   
      <Input placeholder="표준공정코드를 입력하세요." 
      name="spCode"
      ...
      <Input placeholder="표준공정명을 입력하세요."
      name="spName" 
      ...
      <Input placeholder="표준공정시간을 입력하세요."
      name="spTime" 
      ...
      <Input placeholder="표준공정내용을 입력하세요."
      name="spDescription"
