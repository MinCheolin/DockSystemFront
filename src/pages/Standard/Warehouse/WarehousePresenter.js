...
const WarehousePresenter = ({
    ...
    const codeFilter = [...new Set(warehouses.map(item => item.whCode))].map(code => ({
        text: code,
        value: code,
    }));
     const nameFilter = [...new Set(warehouses.map(item => item.whName))].map(name => ({
        text: name,
        value: name,
    }));

    const typeFilter = [...new Set(warehouses.map(item => item.whType))].map(type => ({
        text: type,
        value: type,
    }));
